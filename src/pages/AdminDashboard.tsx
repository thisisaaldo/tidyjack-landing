...
const AdminDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'bookings' | 'customers' | 'payments' | 'photos' | 'completed'>('overview');
  ...
  const uploadPhoto = async (photoBlob: Blob, bookingId: number, photoType: 'before' | 'after') => {
    try {
      ...
      const uploadResponse = await fetch('/api/admin/photos/direct-upload', {
        ...
        body: photoBlob,
        headers: {
          'Content-Type': photoBlob.type || 'image/jpeg', // Use actual blob type
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {!loading && !error && (
          <>
            ...
            {/* Completed Jobs Tab */}
            {activeTab === 'completed' && dashboardData && (
              <div className="space-y-6">
                <div className="bg-white shadow rounded-lg">
            ...
            {activeTab !== 'overview' && activeTab !== 'photos' && activeTab !== 'completed' && (
              <div className="text-center py-12">
